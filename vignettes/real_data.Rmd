...
```{r,echo=TRUE,warning=FALSE,message=FALSE}
new <- testClusters(liver.cnts,cluster_ids=liver$Cluster)
...
clusters <- scSHC(kot.cnts,batch=kot$batch)
...
new <- testClusters(kot.cnts,batch=kot$batch,cluster_ids=Idents(data.seurat))
